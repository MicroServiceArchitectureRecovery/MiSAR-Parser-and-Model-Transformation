            ...
            if isinstance(_declarator, javalang.tree.VariableDeclarator):
                if _declarator.initializer:
                    ...
                    if isinstance(_declarator.initializer, javalang.tree.MemberReference):
                        ...
                        if referenced_field:
                            field_value = evaluate_field(referenced_field, qualifier, properties, module_name,
                                                         app_root_dir)
                            return field_value
            break
    return field_value
    ...
    if not txt_proj_name.get().strip():
                ...
                if 'build' in docker_compose_dict[container_name] or 'image' in docker_compose_dict[container_name]:
                    ...
                    if 'image' in docker_compose_dict[container_name]:
                        if application_containers[container_name]['image'] == '':
                            application_containers[container_name]['image'] = docker_compose_dict[container_name][
                                'image']
                    if 'build' in docker_compose_dict[container_name]:
                        if application_containers[container_name]['build'] == '':
                            if isinstance(docker_compose_dict[container_name]['build'], str):
                                application_containers[container_name]['build'] = docker_compose_dict[container_name][
                                    'build']
                            elif isinstance(docker_compose_dict[container_name]['build'], dict):
                                if 'context' in docker_compose_dict[container_name]['build']:
                                    application_containers[container_name]['build'] = \
                                    docker_compose_dict[container_name]['build']['context']
                    if 'logging' in docker_compose_dict[container_name] or 'log_opt' in docker_compose_dict[
                        container_name]:
        ...
        for container_name in application_containers:
            ...
            for port in application_containers[container_name]['ports']:
                ports = metamodel.DockerContainerPort(ExposesPortsField=port,
                                                      ParentProjectName=application_containers[container_name][
                                                          'parent'],
                                                      ArtifactFileName=application_containers[container_name][
                                                          'filename'])
                container.ports.append(ports)
            order = 0
            for link in application_containers[container_name]['links']:
                order = order + 1
                links = metamodel.DockerContainerLink(DependencyOrder=order, LinksDependsOnField=link,
                                                      ParentProjectName=application_containers[container_name][
                                                          'parent'],
                                                      ArtifactFileName=application_containers[container_name][
                                                          'filename'])
        ...
        for module_name in multi_module_project['modules']:
            ...
            for app_config_dir in app_config_dirs:
                ...
                if application_name:
                    module_properties += get_property_list(application_name, app_config_dir, app_root_dir,
                                                           application_name)
            for property_document in module_properties:
                for config_property in property_document:
                    config_property['value'] = evaluate_property_local_variable1(config_property['value'])
                    config_property['value'] = evaluate_property_local_variable2(config_property['value'],
                                                                                 property_document, module_properties)
                                ...
                                for _type in tree.types:
                                    if isinstance(_type, javalang.tree.ClassDeclaration) or isinstance(_type,
                                        ...
                                        if _type.annotations:
                                            for annotation in get_annotations(_type, multi_module_project['modules'][
                                            ...
                                            for _declaration in _type.body:
                                                ...
                                                for path, _field_declaration in _declaration.filter(
                                                    ...
                                                    if _field_declaration.annotations:
                                                        for annotation in get_annotations(_field_declaration,
                                                            ...
                                                            # parse field value
                                                    java_field.FieldValue = evaluate_field(_field_declaration,
                                                                                           _type.name,
                                                                                           multi_module_project[
                                                                                               'modules'][module_name][
                                                                                               'properties'],
                                                    ...
                                                    if _field_declaration.type:
                                                        ...
                                                        if isinstance(_field_declaration.type,
                                                ...
                                                for path, _method_declaration in _declaration.filter(
                                                    ...
                                                    if _method_declaration.return_type:
                                                        ...
                                                        if element_identifier.lower() in ['string', 'boolean']:
                                                            java_data_type.JsonSchema = '{"title":"' + element_identifier + '","type":"' + element_identifier.lower() + '"}'
                                                        if isinstance(_method_declaration.return_type,
                                                                      javalang.tree.ReferenceType):
                                                            java_data_type.IsPrimitive = False
                                                            if element_identifier.lower() not in ['string', 'boolean']:
                                                                java_data_type.JsonSchema = '{"title":"' + element_identifier + '","type":"object","properties":{}}'
                                                            ...
                                                            if isinstance(_parameter, javalang.tree.FormalParameter):
                                                                ...
                                                                if _parameter.annotations:
                                                                    for annotation in get_annotations(_parameter,
                                                                        ...
                                                                        java_annotation.ParentProjectName = module_name
                                                                        java_annotation.ArtifactFileName = java_file
                                                                        java_annotation.AnnotationName = annotation[
                                                                            'name']
                                                                        for parameter in annotation['parameters']:
                                                                            annotation_parameter = metamodel.JavaAnnotationParameter()
                                                                            annotation_parameter.ParentProjectName = module_name
                                                                            annotation_parameter.ArtifactFileName = java_file
                                                                            annotation_parameter.ParameterName = \
                                                                            parameter['name']
                                                                            if not annotation_parameter.ParameterName:
                                                                                annotation_parameter.ParameterName = 'NOT_AVAILABLE'
                                                                            annotation_parameter.ParameterValue = \
                                                                            parameter['value']
                                                                            if not annotation_parameter.ParameterValue:
                                                                                annotation_parameter.ParameterValue = 'NOT_AVAILABLE'
                                                                            java_annotation.parameters.append(
                                                                                annotation_parameter)
                                                                        java_method_parameter.annotations.append(
                                                                            java_annotation)
                                                                ...
                                                                if _parameter.type:
                                                                    ...
                                                                    java_data_type.IsPrimitive = True
                                                                    java_data_type.JsonSchema = '{"title":"' + element_identifier + '","type":"number"}'
                                                                    if element_identifier.lower() in ['string',
                                                                                                      'boolean']:
                                                                        java_data_type.JsonSchema = '{"title":"' + element_identifier + '","type":"' + element_identifier.lower() + '"}'
                                                                    if isinstance(_parameter.type,
                                                                                  javalang.tree.ReferenceType):
                                                                        java_data_type.IsPrimitive = False
                                                                        if element_identifier.lower() not in ['string',
                                                                                                              'boolean']:
                                                                            java_data_type.JsonSchema = '{"title":"' + element_identifier + '","type":"object","properties":{}}'
                                                                        for _import in imports:
                                                                            parts = _import.split('.')
                                                            ...
                                                            for path, _local_variable in _element.filter(
                                                                ...
                                                                if _local_variable.type:
                                                                    ...
                                                                    if element_identifier.lower() in ['string',
                                                                                                      'boolean']:
                                                                        java_data_type.JsonSchema = '{"title":"' + element_identifier + '","type":"' + element_identifier.lower() + '"}'
                                                                    if isinstance(_local_variable.type,
                                                                                  javalang.tree.ReferenceType):
                                                                        java_data_type.IsPrimitive = False
                                                                        if element_identifier.lower() not in ['string',
                                                                                                              'boolean']:
        ...
        if 'ecore:EPackage' in psm_ecore_dict:
            ...
            if '@nsURI' in psm_ecore_dict['ecore:EPackage'] and '@name' in psm_ecore_dict['ecore:EPackage']:
                xsi_schemaLocation = psm_ecore_dict['ecore:EPackage']['@nsURI'] + ' ' + \
                                     psm_ecore_dict['ecore:EPackage']['@name'] + '.ecore'
...
lst_docker_compose = tkinter.Listbox(frame_docker_compose, width=box_width, height=10,
                                     xscrollcommand=xscroll_docker_compose.set,
                                     yscrollcommand=yscroll_docker_compose.set, foreground=fg_color)
...
lst_app_build = tkinter.Listbox(frame_app_build, width=box_width, height=10, xscrollcommand=xscroll_app_build.set,
                                yscrollcommand=yscroll_app_build.set, foreground=fg_color)
...
lst_module_build_dir = tkinter.Listbox(frame_module_build_dir, width=box_width, height=10,
                                       xscrollcommand=xscroll_module_build_dir.set,
                                       yscrollcommand=yscroll_module_build_dir.set, foreground=fg_color)
...
lst_module_build = tkinter.Listbox(frame_module_build, width=box_width, height=10,
                                   xscrollcommand=xscroll_module_build.set, yscrollcommand=yscroll_module_build.set,
                                   foreground=fg_color)
...
lst_app_config_dir = tkinter.Listbox(frame_app_config_dir, width=box_width, height=10,
                                     xscrollcommand=xscroll_app_config_dir.set,
                                     yscrollcommand=yscroll_app_config_dir.set, foreground=fg_color)
