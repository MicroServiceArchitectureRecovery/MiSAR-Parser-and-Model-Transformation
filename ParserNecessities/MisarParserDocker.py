...
def dockerComposeAnalysis(docker_compose_files, multi_module_project_name):
...
def mergeDockerfileAnalysisDockerCompose(application_containers, app_root_dir):
...
def createDockerPSMElements(application_containers, application, metamodel):
    for container_name in application_containers:
